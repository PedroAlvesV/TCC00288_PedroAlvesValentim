...
RETURNS TABLE(perguntaID int, frequencia float[][]) AS $$
    ...
    BEGIN
        ...
        FOR perguntaCorrente IN select * from pergunta LOOP
            select count(*) from resposta where pergunta = perguntaCorrente.numero into totalRespostas;
            FOR respostaCorrente IN select * from resposta where pergunta = perguntaCorrente.numero LOOP
                select count(*) from escolha where pergunta = perguntaCorrente.numero and resposta = respostaCorrente.numero and pesquisa = p_pesquisa into frequenciaCorrente;
